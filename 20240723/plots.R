...
jobs <- sacct[grepl("^concat-", JobName) & Partition != "nx4",
              .(JobName,
                start=Start,
                partition=Partition,
                elapsed=hms(Elapsed),
                work.dir=sub("^.*/([^/]+)$", "\\1", WorkDir),
                download = ifelse(grepl("-ms3-", JobName),
                                  "mountpoint-s3",
                                  "AWS-CLI")
                )]
...
p.time.vs.sample <- ggplot(data) +
    aes(sample, elapsed) +
    geom_violin(draw_quantile=.5) +
    geom_point(aes(color=partition, shape=download)) +
...
p.time.vs.start <- ggplot(data) +
    facet_grid(sample ~ .) +
    aes(start, elapsed, shape=download, color=partition) +
    geom_point() +
    scale_colour_brewer(type="qual") +
    theme_bw() +
    theme(axis.text.x = element_text(angle=30, hjust=1))
